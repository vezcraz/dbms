...
select oisbn from OrderBook 
GROUP BY oisbn having count(*)>2;
select * from OrderBook;
select ocid, (timestampdiff(year,max(orderdate),'2020-02-19')) as diff from OrderBook
GROUP BY ocid having diff<7 ;
...
where oisbn in 
(select isbn from Book 
where title="Operating Systems" or title = "Data Structures and Algorithms")
GROUP BY ocid having count(*)=2
)
;
...
(
	select ocid from OrderBook 
	group by ocid having sum(qty)<4
);

-- q10
select isbn, qty_in_stock from Book as a
where EXISTS
(
	select isbn from Book as b
	where not EXISTS
	(
		select isbn from Book as c where qty_in_stock>=all(select qty_in_stock from Book) and b.isbn=c.isbn
	)
	and a.isbn=b.isbn 
)
and qty_in_stock>=all(select qty_in_stock from Book as a
where EXISTS
(
	select isbn from Book as b
	where not EXISTS
